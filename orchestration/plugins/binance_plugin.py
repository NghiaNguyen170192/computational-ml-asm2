...
from utils import Utils  # adjust path if needed
...
def upsert_csv_from_minio():
    ...
    # List only raw klines files (ignore subfolders)
    keys = [
        k for k in s3_hook.list_keys(bucket_name=bucket_name, prefix="binance/klines/")
        if not ("/inprogress/" in k or "/processed/" in k)
    ]

    if not keys:
        print(f"No raw files found in 'binance/klines/'")
        return
    ...
    sorted_keys = sorted(keys, key=extract_timestamp)
    ...
    for object_key in sorted_keys:
        ...
        # Move to INPROGRESS
        inprogress_key = f"binance/klines/inprogress/{filename}"
        s3_hook.copy_object(
            source_bucket_name=bucket_name,
            source_bucket_key=object_key,
            dest_bucket_name=bucket_name,
            dest_bucket_key=inprogress_key
        )
        ...
        try:
            # Load into memory
            obj = s3_hook.get_key(key=inprogress_key, bucket_name=bucket_name)
            ...
            # Move to PROCESSED only after DB success
            processed_key = f"binance/klines/processed/{filename}"
            s3_hook.copy_object(
                source_bucket_name=bucket_name,
                source_bucket_key=inprogress_key,
                dest_bucket_name=bucket_name,
                dest_bucket_key=processed_key
            )

            s3_hook.delete_objects(
                bucket_name=bucket_name, keys=[inprogress_key])

            print(f"Completed {filename}, moved to {processed_key}")
        ...
        with conn.cursor() as cursor:
            execute_values(cursor, """
                INSERT INTO binance_klines (
                    ...
                    taker_buy_base_volume, taker_buy_quote_volume, status
                )
                VALUES %s
